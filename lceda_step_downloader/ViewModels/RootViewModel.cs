...
namespace lceda_step_downloader.ViewModels
{
    public class RootViewModel : PropertyChangedBase
    {
        ...
        private static readonly string DefaultStepDirectory = Path.Combine(AppContext.BaseDirectory, "step");
        ...
        private string _stepSaveDirectory;
        public string StepSaveDirectory
        {
            get { return _stepSaveDirectory; }
            set { SetAndNotify(ref _stepSaveDirectory, value); }
        }
        ...
        public RootViewModel()
        {
            ...
            Directory.CreateDirectory(DefaultStepDirectory);
            StepSaveDirectory = DefaultStepDirectory;
        }
        ...
        public void DownloadObj()
        {
            ...
            var objFilePath = Path.Combine(TempDirectory, GetSafeFileName(Selecteditem.title) + ".obj");
            if (File.Exists(objFilePath))
            {
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    MyModelGroup = CurrentHelixObjReader.Read(objFilePath);
        ...
        {
            DownloadStepToDirectory(StepSaveDirectory);
        }

        public void BrowseStepSaveDirectory()
        {
            using Forms.FolderBrowserDialog dialog = new()
            {
                Description = "选择STEP模型保存文件夹",
                SelectedPath = Directory.Exists(StepSaveDirectory) ? StepSaveDirectory : DefaultStepDirectory,
                UseDescriptionForTitle = true
            };

            if (dialog.ShowDialog() == Forms.DialogResult.OK)
            {
                StepSaveDirectory = dialog.SelectedPath;
            }
        }

        public void DownloadStepToDirectory(string saveDirectory)
        {
            ...
            //封装名称
            string fileToWriteTo = GetStepFilePath(saveDirectory);
            ...
            Task.Run(() => DownloadStepToDirectoryAsync(saveDirectory));
        }

        //构造PCB数据, 以利用lceda专业版的PCB导出STEP接口
        public Task<string> DownloadStepAsync()
        {
            return DownloadStepToDirectoryAsync(StepSaveDirectory);
        }

        public async Task<string> DownloadStepToDirectoryAsync(string saveDirectory)
        {
        ...
        public async void ObjMtlSplit(Task<Stream> objstream)
        {
            ...
            StreamWriter objWriter = new(Path.Combine(TempDirectory, tempTitle + ".obj"));
            StreamWriter mtlWriter = new(Path.Combine(TempDirectory, tempTitle + ".mtl"));
            ...
            {
                ObjReader CurrentHelixObjReader = new();
                MyModelGroup = CurrentHelixObjReader.Read(Path.Combine(TempDirectory, tempTitle + ".obj"));
            });
        }

        private string GetStepFilePath(string saveDirectory)
        {
            if (string.IsNullOrWhiteSpace(saveDirectory))
            {
                saveDirectory = DefaultStepDirectory;
            }

            Directory.CreateDirectory(saveDirectory);
            var stepTitle = GetSafeFileName(Selecteditem.footprint.display_title);
            return Path.Combine(saveDirectory, stepTitle + ".step");
        }
